...
class Storage:
    ...
    def thread_do(self):
        ...
        """
        # インスタンス生成
        self.__db = DbMssql.DbMssql()
        # データベースへの書き込み処理
        while True:
            #
            # SQLServerの生死確認
            #
            if self.__db.is_connectable() == False:
            ...
            while not self.qsigtower.empty():
                ...
                # SQLServerに登録
                self.Append(ccm[buf[1]], buf[0], buf[2])
            #
            # STEP2:作業者状態
            #
            # キューを順次呼び出し
            while not self.qworkerstat.empty():
                ...
                # SQLServerに登録
                self.Append("WorkerStat4.mLI", buf[0], buf[1])
            #
            # STEP3:CT
            #
            # キューを順次呼び出し
            while not self.qct.empty():
                buf = self.qct.get()
                # SQLServerに登録
                self.Append("CT.mLI", buf[0], buf[1])
            #
    ...
    def Append(self, arg_CCM, arg_val, arg_date):
        """
        レコードの追加
        """
        ret = self.__db.AddToMssql(("%s" % os.uname()[1]),
                                   arg_val,
                                   ("%s" % (arg_CCM)),
